...
public class BlueSideFar extends OpMode {
    ...
    private final Pose outtakePre = new Pose(144-90,16,Math.toRadians(90));
    ...
    private final Pose end = new Pose(144-110,8,Math.toRadians(180));
    ...
    public void buildPaths(){
        ...
        End = follower.pathBuilder()
                .addPath(new BezierLine(outtakePre,end))
                .setConstantHeadingInterpolation(Math.toRadians(180))
                .build();
    }
    ...
    public void statePathUpdate(){
        ...
        switch(pathState){
            case PRELOAD:
                if(!moving && pathTimer.getElapsedTime() > 3000) {
                    turret.setTargetPosition(130);
                    follower.followPath(Preload,0.5, true);
                    moving = true;
                }
                if(!follower.isBusy() && pathTimer.getElapsedTime() > 3000 + 50) {
