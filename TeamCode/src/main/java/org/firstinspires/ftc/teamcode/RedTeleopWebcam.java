...
public class RedTeleopWebcam extends LinearOpMode {
    ...
    public void runOpMode() {
        // Ensure any leftover portal is closed before starting
        ...
        // Start a managed camera thread so we can shut it down cleanly later
        cameraThread = new Thread(() -> {
            cameraControls();
        }, "CameraControlsThread");
        ...
        while (opModeIsActive()) {
            ...
            List<AprilTagDetection> detectedTags = (aprilTag != null) ? aprilTag.getDetections() : Collections.emptyList();
        ...
        // Stop camera thread and join before closing resources
    ...
    public void cameraControls() {
        // Single-shot control after a short delay. Make null-safe and interruption-aware.
        try {
            Thread.sleep(3000);
        ...
        try {
            ...
            // don't let camera-control errors crash the opmode
            try {
                telemetry.addData("cameraControlsError", e.toString());
                telemetry.update();
            } catch (Exception ignored) {
            }
        } finally {
            // cameraControls completes; thread will exit
        }
    }
